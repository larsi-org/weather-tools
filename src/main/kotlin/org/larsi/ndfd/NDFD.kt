...
class NDFD
{
	companion object {
		...
		val sensorIDs = intArrayOf( // SENSOR_CNT
			2,  // ICAO Temperature (predicted)
			5,  // ICAO Dew Point Temperature (predicted)
			23, // ICAO Wind Speed (predicted)
			20, // ICAO Wind Direction (predicted)
			11, // ICAO Clouds (predicted)
			8,  // ICAO Humidity (predicted)
		)
		...
		fun check() {
			try {
				...
				entries.addAll(md.queryList("SELECT Prefix, Y(Location) AS Latitude, X(Location) AS Longitude FROM location") {
					NDFDEntry(it.getString(1).lowercase(), it.getString(2), it.getString(3))
	...
	fun update(md: MeteredDataConnector, prefix: String)
	{
		try {
			...
			for (j in 0 until SENSOR_CNT) {
				...
				try {
					md.addBatch(md.emptyLogSQL(prefix, sensorIDs[j]))
					val v = values[j]
					val t = times[j]
					val lastCurrent = md.getMaxDateTimeLog(prefix, sensorIDs[j] - 1)
					if (prefix.startsWith("k")) {
						val cnt = if (v.size < t.size) v.size else t.size
						for (i in 0 until cnt) {
							val tPredicted = t[i]
							if (tPredicted > lastCurrent)
								md.addBatch(md.insertLogSQL(prefix, tPredicted, sensorIDs[j], v[i]))
						}
					}
